...
public class TipLightView extends View {
    ...
    private float mHeight;//传进来的View的高度
    private float mWidth;//传进来的View的宽度
    ...
    private void initViewOption() {
        ...
        //通过Measure计算View宽高
        mView.measure(w, h);
        mHeight = mView.getMeasuredHeight()-getPaddingTop()-getPaddingBottom();
        mWidth = mView.getMeasuredWidth()-getPaddingLeft()-getPaddingRight();
    }
        ...
        switch (lightType) {
            case Rectangle:
                srcCanvas.drawRoundRect(
                        new RectF(mX - mLRPadding, mY - mTBPadding, mX + mWidth + mLRPadding,
                                mY + mHeight + mTBPadding), roundRadius, roundRadius, mPaint);
                break;
            case Circle:
                srcCanvas.drawCircle(mX + (mWidth / 2), mY + (mHeight / 2),
                        (mWidth > mHeight ? mWidth + mLRPadding : mHeight + mTBPadding) / 2, mPaint);
                break;
            case Oval:
